...
shared_ptr<demogData> readCSVLineDemog(std::string theLine) {
    ...
    raceDemogData inRD(FirstNation*totalPop2014,Asian*totalPop2014,Black*totalPop2014,Latinx*totalPop2014,HIPacificIsle*totalPop2014,
    MultiRace*totalPop2014,White*totalPop2014,WhiteNH*totalPop2014,totalPop2014);
 
    //update as needed this is lab3 version
    return make_shared<demogData>(name, state, popOver65, popUnder18,
            popUnder5, bachelorDegreeUp, highSchoolUp, belowPoverty, totalPop2014, inRD);
}
...
std::vector<shared_ptr<regionData> > read_csv(std::string filename, typeFlag fileType)
{
    ...
    if(myFile.good()) {
        ...
        while(std::getline(myFile, line)) {
            if (fileType == DEMOG)
            {
                theData0.push_back(readCSVLineDemog(line));
            }
            else if (fileType == POLICE) 
            {
                theData0.push_back(readCSVLinePolice(line));
            } 
            else 
            {
